...
### Atlassian — OAuth is impossible; the API token works

This was the biggest surprise and it is settled by Atlassian's own discovery document at
...
The viable path is a user-created scoped API token used as HTTP Basic with their email. One token
carries both Jira and Confluence scopes, read-only scopes exist for both, and the credential
therefore expresses Chief's read-only posture rather than merely promising it. Scoped tokens must
be called against `api.atlassian.com/ex/jira/{cloudId}`, never `<site>.atlassian.net` — mixing the
two is a common source of 401s.
...
### Slack — trivial auth, crippled data access
...
| 3   | Jira + Confluence       | Pasted scoped API token, HTTP Basic | 7     | ~8 user setup steps; admins can block tokens         |
| 4   | Gmail                   | Bring-your-own Cloud project        | 8     | Annual paid CASA assessment otherwise                |
| 5   | Slack                   | PKCE, but bring-your-own app        | 9     | Distributed apps throttled to 1 req/min, 15 messages |
...
Ranks 2 and 3 may swap: Google Calendar reuses machinery already built and is cheaper in code, but
requires review paperwork that Atlassian does not.
...
This is why `integration_accounts.client_id` exists from the first migration: adopting that path
...
- `credential_kind` — `oauth` or `token`. Explicit, so Atlassian's pasted credential is not
...
- `client_id` — NULL means Chief's own registration; set means bring-your-own.
