...
| **Settings still reads three other things once per card**      | REC-42 took `connections()` from 5 reads to 1 by lifting `useIntegrations` to the screen. Measured on the same render afterwards: `sync_status` 5, `sign_in_registrations` 2, `run_doctor` 2. Same shape, same cure, and deliberately out of that issue's scope — `useSyncState` also serves the header, which is outside `SettingsView` and genuinely wants its own                                                                                                                                                   | REC-42      |
| **The OS ships a model Chief does not use**                    | Apple's Foundation Models and Windows' on-device model are free and already installed. Studied in `docs/on-device-intelligence.md` (2026-09-14): not for the agent loop — Windows offers no tool calling and Apple's 4,096-token window would cap a Standard machine at Light — and not yet, since Phi Silica is removed in November 2026. Worth revisiting for the three single-shot call sites                                                                                                                       | REC-31      |
...
| **Jira and Confluence Data Center**                            | The token path is **Atlassian Cloud only**, decided rather than overlooked. Data Center's credential is a Personal Access Token — a different object from a Cloud API token — and authenticates as `Authorization: Bearer <token>` with no email. The change is small (an empty email meaning `Bearer`) and is left out because nobody can test it here; a second auth scheme that has never met a real server is a branch that looks supported and is not. Accommodate it when there is an instance to try it against | REC-22      |
